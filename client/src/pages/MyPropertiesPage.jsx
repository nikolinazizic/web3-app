...
function MyProperty() {
    ...
    const { address, chainId, isConnected } = contextValue;
    ...
    const getPropertyDetails = async() =>{
      try{
        ...
        const propertyDetails = await contract.listPropertiesByOwner(address);
    ...
    const submitProperty = async(e) =>{
      ...
      try{
        setProcessing(true);
          ...
          const signer = await ethersProvider.getSigner();
          const contract = new Contract(contractAddress, PROPERTY_REGISTRY_ABI, signer);
          await contract.registerProperty(address,propertyName,category,location,area,description,img,price).then((result)=>{
            setShow(false);
            notify();
          })
          setProcessing(false);
 ...
 {address != undefined ? <div className="flex justify-end items-end mt-3">
<button type='button' className=' w-auto ml-2 mt-2 bg-blue-400 hover:bg-blue-400 text-white font-bold py-2 px-4 rounded-full'onClick={handleShow}> + Add</button>
</div>:<></>}
 {address!== undefined? (
          <div className='flex flex-col items-center justify-center mt-6 lg:mt-20'>
            <h2 className="text-4xl font-semibold mb-4 mx-auto text-center">{search === ''? 'Results for "All properties"' : `Results for "${search}"`}</h2>
            <h2 className="text-lg mb-4 text-neutral-400 mx-auto">Filter by categories:</h2>
        ...
        {address != undefined ? (
          <div className="flex items-center justify-center mt-3">
            <button
              type="button"
              className="w-28  ml-2 mt-2 bg-blue-400 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-full  focus:bg-neutral-400"
              onClick={() => setSearch("")}
            >
              All
            </button>
          ...
          <div className="max-w-7xl mx-auto pt-10 px-6 pb-10">
            <div className="container mx-auto px-4">
              <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-8">
                {propertyDetails.filter(result => result[3].includes(search)).map((result) =>
                  <div key={result[1]} className="bg-white rounded-lg shadow-lg shadow-blue-200/50 p-8 hover:scale-95 transition-all duration-300">
                    <div className="relative overflow-hidden">
                      <img className="object-cover w-full h-37 sm:h-52" src={result[7]} alt="Product"/>
                      <div className="absolute inset-0 bg-black opacity-40"></div>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <Link to={'/property/' + parseInt(result[1])}>
                          <button className="bg-white text-neutral-700 py-2 px-6 rounded-full font-bold hover:bg-blue-300">View Details</button>
                        </Link>
                      </div>
                    </div>
                    <h3 className="text-xl font-semibold text-neutral-700 mt-4">{result[2]}</h3>
                    <div className="mt-2">
                    <span className="font-semibold  text-sm text-neutral-700">Owner</span>
                    <p className="text-gray-500 text-sm mt-2 break-words">{result[0]}</p>
                    </div>
                    <div className="mt-2">
                    <span className="font-semibold  text-sm text-neutral-700">Category</span>
                    <p className="text-gray-500 text-sm mt-2 break-words">{result[3]}</p>
                    </div>
                    <div className="flex items-center justify-between mt-4">
                      <span className="text-neutral-700 font-bold text-lg">{result[5]}</span>
                      <span className="text-neutral-700 font-bold text-lg">{parseInt(result[8]) + ' ETH'}</span>
                    </div>
                  </div>
                )}
              </div>
  ...
  <div className="absolute top-40 left-0 bg-opacity-50 w-full flex justify-center items-center">
      <div className="grid rounded-lg shadow-xl md:w-[60%] lg:w-[45%] w-[80%] bg-white">
        ...
        <div className="flex justify-center">
          <div className="flex">
            <h1 className=" font-bold md:text-2xl text-xl text-neutral-800">Create new property</h1>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
          ...
          <div className="grid grid-cols-1">
            ...
            <input id="propertyPrice" className="py-2 px-3 bg-white text-neutral-600 rounded-lg border-2 border-blue-200 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent" type="text" placeholder="Property price" value={price} onChange={(e)=>setPrice(e.target.value)} required />
          </div>
        </div>
        <div className="grid grid-cols-1 mt-5 mx-7">
...
<div className="flex items-center justify-center mt-5 mx-20">
  {imageFile != '' ? <img src={imageFile} className="rounded-md" /> : <></>}
        ...
        <div className='flex items-center justify-center  md:gap-8 gap-4 pt-5 pb-5'>
          <button className='w-28  ml-2 text-lg bg-gray-400 hover:bg-gray-600 py-2 px-4 rounded-full' onClick={()=>setShow(false)}>Cancel</button>
          <input type="submit" className="w-28 text-lg ml-2 bg-blue-400 hover:bg-blue-600 py-2 px-4 rounded-full cursor-pointer focus:bg-neutral-400" value={processing ? "Processing" : "Submit"} disabled={processing} />
        </div>
      </div>
    </div></form>:<></>}
 </>
